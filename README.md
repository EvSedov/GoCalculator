...
```
3. Собрать проект командой `docker-compose build`
4. Запустить проект командой `docker-compose up`
  - Web-сервер запуститься на http://127.0.0.1/
  - Оркестратор запустится на http://127.0.0.1:81, все добавленные выражения можно посмотреть перейдя по пути `/expressions` для данного `URL`
  - Вычислитель запустится на http://127.0.0.1:85, посмотреть текст о том, что вычислитель запустился можно перейдя по пути `/calc` для данного `URL`
