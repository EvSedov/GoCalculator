...
Для проверки работы проекта необходимо, после старта проекта, в браузере открыть три вкладки по указанным адресам запуска: web-сервера (http://127.0.0.1:8080), оркестратора (http://127.0.0.1:81/expressions) и вычислителя (http://127.0.0.1:85/start).  
